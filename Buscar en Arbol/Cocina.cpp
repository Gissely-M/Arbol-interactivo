...
// Buscar y mostrar ruta(s) de un valor dado
template <typename T>
bool buscarYMostrarRuta(Nodo<T>* raiz, T valor, string ruta, int& contador) {
    if (!raiz) return false;

    bool encontrado = false;
    if (raiz->valor == valor) {
        contador++;
        cout << "Ruta " << contador << ": " << ruta << endl;
        encontrado = true;
    }

    bool izq = buscarYMostrarRuta(raiz->izquierdo, valor, ruta + " -> Izq", contador);
    bool der = buscarYMostrarRuta(raiz->derecho, valor, ruta + " -> Der", contador);
...
// Funcion general que maneja todo: buscar, mostrar ruta, recorrer
template <typename T>
void buscarYMostrarRuta(Nodo<T>* raiz, T valor) {
    vector<T> camino;
    if (buscarValorConRuta(raiz, valor, camino)) {
        cout << "\nRuta encontrada: ";
        ...
        recorrerRutaInteractiva(camino);
    } else {
        cout << "El valor no fue encontrado en el arbol." << endl;
    }
}
        ...
        do {
            ...
            switch (menuOpcion) {
                case '1': {
					...
					buscarYMostrarRuta(raiz, buscarValor);
                ...
                case '2': {
					...
					cout << endl;
					recorrerRutaInteractiva(rutaMenor);
                    break;
                }
                case '3': {
                    if (!raiz) break;
					...
					cout << endl;
					recorrerRutaInteractiva(rutaMayor);
                    break;
                }
                case '4':
